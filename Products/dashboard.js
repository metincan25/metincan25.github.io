// dashboard.js - NİHAİ VE TAM VERSİYON
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Bu fonksiyonu global hale getirerek her yerden çağrılmasını sağlıyoruz
    ...
    // Kullanıcının rolüne göre arayüzü ayarlayan fonksiyon
function applyRoleBasedUI() {
    const token = localStorage.getItem('authToken');
    if (!token) return;

    const decodedToken = parseJwt(token);
    // Token içindeki rol bilgisini alıyoruz. Rol claim'inin tam adını token'ınıza göre ayarlamanız gerekebilir.
    const userRole = decodedToken ? decodedToken['http://schemas.microsoft.com/ws/2008/06/identity/claims/role'] : null;

    if (userRole !== 'Admin') {
        // Eğer kullanıcı "Admin" değilse, "admin-only" class'ına sahip tüm elementleri gizle.
    ...
    function updateUserInfo() {
        ...
        if (token) {
            const decodedToken = parseJwt(token);
            if (decodedToken) {
                // Token'daki 'sub' (subject) claim'i genellikle kullanıcı adını tutar.
                const username = decodedToken.sub; 

                // İsimden baş harfleri al (Örn: Metin Can Gül -> MG)
                const nameParts = username.split(' ');
                const initials = nameParts.length > 1 
                    ? nameParts[0][0] + nameParts[nameParts.length - 1][0]
                    : username.substring(0, 2);

                // HTML elemanlarını bul ve içeriklerini değiştir
   ...
   const summaryTabButton = document.querySelector(".nav-link[data-page='dashboard-summary']");
    if(summaryTabButton){
        summaryTabButton.addEventListener('click', function(e){
            e.preventDefault();
            // Bu özel bir sekme olduğu için openTab çağırmasına gerek yok, zaten hep açık.
            // Sadece Bootstrap'in sekme gösterme özelliğini kullanabiliriz.
   ...
   window.initializeProductList = initializeProductList;
    ...
    confirmDeleteButton.addEventListener('click', async () => {
        ...
        deleteModalBody.textContent = ''; // Hata mesajını temizle
    ...
    if (!orderToShipId) return;
     ...
     try {
        ...
        shipmentModal.hide(); // Onay modalını kapat
        
        // Genel başarı modalını göster
        window.showSuccessModal(`'${orderToShipNumber}' siparişi için '${result.waybillNumber}' numaralı irsaliye oluşturuldu.`);

        // "Sevk Edilecekler" listesinin kendini yenilemesi için sekmeyi yeniden açtırabiliriz.
...
function renderDocumentTemplate(data) {
    ...
    // API'den gelen verinin temel yapısını kontrol et
    ...
    // 1. Belge tipine göre fiyat gösterilip gösterilmeyeceğine karar ver
    const isWaybill = (data.documentType || '').toLowerCase().includes('irsaliye');

    // 2. Kolon başlıklarını dinamik olarak oluştur
    ...
    // 3. Ürün satırlarını (kalemleri) dinamik olarak oluştur
    const itemsHtml = data.items.map(item => {
        if (isWaybill) {
            // İrsaliye için fiyatsız satır
            ...
            // Fatura ve Sipariş için fiyatlı satır
            return `<tr class="item">
                        <td>${item.sku || ''}</td>
                        <td>${item.productName || ''}</td>
                        <td class="text-center">${item.quantity}</td>
    ...
    // 4. Genel Toplam satırını dinamik olarak oluştur
    ...
    // 5. Belge başlığını güvenli bir şekilde al
    ...
    function initializeNewProductForm(container) {
     ...
     const unitSelect = container.querySelector('#product-unit-select');
        ...
        async function populateDropdown(selectElement, endpoint, defaultOptionText) {
            try {
                ...
                selectElement.innerHTML = `<option value="">${defaultOptionText}</option>`; // Varsayılan seçeneği ekle
                data.forEach(item => {
        ...
        // Sayfa yüklendiğinde hem kategorileri hem markaları aynı anda çek
     ...
     form.addEventListener('submit', async (event) => {
        ...
        // Formdaki TÜM alanları okuyoruz
        const newProductData = {
           name: container.querySelector('#product-name')?.value,
            ...
            // Artık metin olarak değil, seçilen ID'yi alıyoruz.
            // Ayrıca 'unitName' gibi eski alanları buradan kaldırıyoruz.
    ...
    function initializeProductList(container) {
        ...
        input.addEventListener('keyup', (event) => {
            // Her tuşa basıldığında API'yi yormamak için bir gecikme mekanizması (debounce)
            clearTimeout(debounceTimer);
            debounceTimer = setTimeout(() => {
                const key = event.target.getAttribute('data-filter-key');
                const value = event.target.value;

                // Eğer input boş ise filtreyi kaldır, doluysa ekle
                if (value) {
                    filters[key] = value;
                } else {
                    delete filters[key];
                ...
                // Filtre değiştiğinde her zaman 1. sayfaya git
                filters.page = 1;
          ...
          const deleteButton = e.target.closest('.delete-product-btn');
            if(editButton) {
             ...
             if (deleteButton) {
                ...
                afterDeleteCallback = fetchAndRenderProducts; // Silme sonrası listeyi yenile
                deleteEndpoint = '/api/products'; // Silme için doğru endpoint'i ayarla
 ...
 function initializeProductEditPage(container, pageUrl) {
    ...
    // Formdaki tüm elemanları en başta seçiyoruz
    ...
    // Yardımcı Fonksiyon: Dropdown menülerini doldurur
    ...
    // Ana Fonksiyon: API'den ürünün mevcut verilerini çeker ve formu doldurur
    async function loadProductDetails() {
        try {
            ...
            // Tüm form alanlarını gelen veriyle doldur
    ...
    // Form gönderildiğinde (Kaydet butonuna basıldığında) güncelleme isteği at
    ...
    // Sayfa ilk açıldığında verileri çekip formu doldur
    loadProductDetails();
 }

 // dashboard.js içindeki bu fonksiyonu bul ve tamamını değiştir

 function initializeHomePage(container) {
   const token = localStorage.getItem('authToken');
    if (!token) { 
        // Bu fonksiyon sadece giriş yapılmışken çağrılmalı, ama yine de kontrol edelim.
        window.location.href = 'invlogin.html';
        return; 
    }

    // --- 1. Gerekli HTML Elemanlarını Seçme ---
    ...
    // Hata kontrolü: HTML'in doğru yüklendiğinden emin olalım.
    if (!welcomeUsernameEl || !liveDateEl || !liveClockEl || shortcutCards.length === 0) {
        console.error("Ana Sayfa için gerekli HTML elemanları bulunamadı. Lütfen index.html dosyasını ve ID'leri kontrol edin.");
        return;
    }

    // --- 2. Yardımcı Fonksiyonlar ---
    
    // JWT Token'ını çözümleyip içindeki verileri (payload) alan fonksiyon
    function parseJwt(token) {
        try {
            return JSON.parse(atob(token.split('.')[1]));
        } catch (e) {
    ...
    // Saati ve tarihi anlık olarak güncelleyen fonksiyon
    function updateDateTime() {
        const now = new Date();
        // Tarihi daha okunaklı bir formatta yazdır: Örn: "18 Eylül 2025, Perşembe"
        liveDateEl.textContent = now.toLocaleDateString('tr-TR', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
        // Saati saniyeyi de gösterecek şekilde yazdır: Örn: "16:30:45"
        liveClockEl.textContent = now.toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit', second: '2-digit' });
    }

    // --- 3. Fonksiyonları Çalıştırma ve Olayları Bağlama ---

    // Hoş geldin mesajını kullanıcının adıyla ayarla
    const decodedToken = parseJwt(token); 
    if (decodedToken && decodedToken.sub) {
        welcomeUsernameEl.textContent = decodedToken.sub;
    }

    // Saati ve tarihi ilk açılışta hemen çalıştır, sonra her saniye güncelle
    updateDateTime();
    const clockInterval = setInterval(updateDateTime, 1000);

    // Hızlı erişim kartlarına tıklama olayı ekle
    shortcutCards.forEach(card => {
        card.addEventListener('click', function() {
            const pageUrl = this.dataset.page;
            const pageTitle = this.dataset.title;
            if (pageUrl && pageTitle) {
                // openTab fonksiyonunun global (window.openTab) olduğundan emin olun
    ...
    function initializeUnitsPage(container) {
        ...
        // Birimleri getiren ve listeleyen fonksiyon
        ...
        fetchAndRenderUnits(); // Sayfa ilk yüklendiğinde birimleri getir
    }

    // dashboard.js içine, diğer initialize... fonksiyonlarının altına ekle
...
function initializeInvoicesPage(container) {
   const token = localStorage.getItem('authToken');
    ...
    // Faturaları getiren ve listeleyen fonksiyon (ARAMA ÖZELLİKLİ)
    async function fetchAndRenderSalesInvoices(search = '') {
        ...
        // Arama parametresini URL'e ekliyoruz
    ...
    // Fatura satırına tıklandığında detay modal'ını aç
    tableBody.addEventListener('click', async (e) => {
       const row = e.target.closest('.sales-invoice-row'); // HTML'deki satır class'ı
        if (row) {
            ...
            docModalActionButtons.innerHTML = ''; // Önceki butonları temizle

            // Eğer fatura "Ödendi" değilse, işlem butonunu ekle
            if (invoiceStatus !== 'Ödendi') {
                const paidButton = document.createElement('button');
                paidButton.className = 'btn btn-sm btn-success';
                paidButton.innerHTML = `<i class="fas fa-check me-1"></i> Tahsil Edildi İşaretle`;
                paidButton.onclick = () => markAsPaid(invoiceId, 'salesinvoices');
                docModalActionButtons.appendChild(paidButton);
            }
            docModal.show();
            ...
            try {
                ...
                renderDocumentTemplate(data); // Genel şablonu çağır
            } catch(error) {
     ...
     function initializeCategoriesPage(container) {
        ...
        categoryList.addEventListener('click', (e) => {
            ...
            if (deleteButton) {
                ...
                // Silinecek öğe bilgilerini ve ne yapılacağını kaydet
                itemToDeleteId = deleteButton.dataset.id;
                itemToDeleteName = deleteButton.dataset.name;
                // Silme başarılı olursa kategori listesini yenile
    ...
    // YENİ FONKSİYON: ÜRÜN İŞLEMLERİ SAYFASI MANTIĞI
    function initializeProductOperationsPage(container) {
        ...
        /*function renderProductTable(products) {
            if (!products || products.length === 0) { productListBody.innerHTML = `<tr><td colspan="4">Ürün bulunamadı.</td></tr>`; return; }
        ...
        // Bir ürün satırına tıklandığında detay modalını aç
        ...
        // Gelen ürün verisiyle detay modalını doldur
        ...
        fetchAndRenderProducts(); // İlk yükleme
    }
    ...
    // Düzenleme modalındaki "Kaydet" butonuna basıldığında
    saveFieldButton.addEventListener('click', async () => {
        ...
        try {
            ...
            editModal.hide();
            detailModal.hide(); // Detay modalını da kapat
            window.showSuccessModal("Ürün başarıyla güncellendi.");
            // Ana tabloyu yenilemek için en baştaki listeyi tekrar çağırmamız lazım.
            // Bu, daha karmaşık bir state yönetimi gerektirir. Şimdilik manuel yenileme varsayalım.
            // Veya ilgili sekmeyi yeniden açtırabiliriz.
     ...
     form.addEventListener('submit', async (e) => {
        ...
        const newCategory = {
            ...
            form.reset();
            await fetchAndRenderCategories(); // Listeyi yenile
    ...
    function initializeBrandsPage(container) {
     ...
     brandList.addEventListener('click', (e) => {
        ...
        if (deleteButton) {
            ...
            //deleteUrl = 'https://localhost:7038/api/brands';
...
function initializeNewOrderPage(container) {
    ...
    let shoppingCart = [];
    let selectedCustomerId = null; // Cari modülü gelene kadar geçici
    ...
    // Fiyat bölümünü seçilen ürüne ve checkbox'a göre yönetir
    ...
    // Alışveriş sepetini (tabloyu ve toplam tutarı) günceller
    ...
    // Ürün giriş formunu ve seçimleri sıfırlar
    ...
    // --- 4. Olay Dinleyicileri (Event Listeners) ---
     customerSearchInput.addEventListener('keyup', (e) => {
        ...
        debounceTimer = setTimeout(async () => {
            try {
                // Sadece "Müşteri" tipindeki carileri ara
    ...
    // ARAMA SONUÇLARINDAN BİR MÜŞTERİ SEÇİLDİĞİNDE
    ...
    // Ürün arama kutusuna yazıldığında API'den ürünleri çeker
    ...
    // Arama sonuçlarından bir ürün seçildiğinde
    ...
    // "Özel Fiyat Kullan" checkbox'ı değiştiğinde
    ...
    // Fiyat listesi dropdown'ı değiştiğinde
    ...
    // "Ekle" butonuna basıldığında ürünü sepete ekler
    ...
    // Ana formu gönderip SİPARİŞİ OLUŞTURMA
    form.addEventListener('submit', async (e) => {
        e.preventDefault();
    ...
    // --- 5. Başlangıç Durumu ---
    renderShoppingCart(); // Sayfa ilk açıldığında boş sepeti göster
}
  ...
  function initializeOrderListPage(container) {
     ...
     // --- 2. Ana Fonksiyon: Siparişleri Getir ve Listele ---
     async function fetchAndRenderSalesOrders() {
        tableBody.innerHTML = `<tr><td colspan="6" class="text-center p-5"><div class="spinner-border text-primary"></div></td></tr>`;
        
        // Tüm filtre inputlarından değerleri toplayıp URL parametresi oluştur
        const params = new URLSearchParams();
        filterInputs.forEach(input => {
            if (input.value) {
                params.append(input.dataset.filterkey, input.value);
    ...
    // Her bir filtre input'una olay dinleyici ekle
    filterInputs.forEach(input => {
        // 'keyup' olayı sadece metin kutuları için
        if (input.type === 'text') {
            input.addEventListener('keyup', () => {
                clearTimeout(debounceTimer);
                debounceTimer = setTimeout(() => {
        ...
        // 'change' olayı ise tarih ve seçim kutuları için
     ...
     tableBody.addEventListener('click', async (e) => {
        ...
        if (shipButton) {
            // "Sevk Et" butonuna basıldıysa, onay modalını aç
            e.stopPropagation();
    ...
    // YENİ EKLENDİ: Onay modalındaki "Evet, Sevk Et" butonunu dinle
    confirmShipmentButton.addEventListener('click', async () => {
        if (!orderToShipId) return;
        ...
        try {
            ...
            await fetchAndRenderSalesOrders(); // Listeyi yenile
...
function initializeWaybillsPage(container) {
    ...
    // Genel belge detay modalı ve elemanları
    const docModal = new bootstrap.Modal(document.getElementById('documentDetailModal'));
    const docModalTitle = document.getElementById('documentModalTitle');
    const docModalContent = document.getElementById('documentModalContent');
    const docModalActionButtons = document.getElementById('documentModalActionButtons');
    
    // Genel onay modalı ve elemanları
    ...
    // Durum Değişkenleri
    let activeWaybillId = null; // İşlem yapılacak aktif irsaliyenin ID'si
    let debounceTimer;
    ...
    // --- 2. Ana Fonksiyon: İrsaliyeleri Getir ve Listele ---
        ...
        if (row) {
            ...
            if (waybillStatus !== 'Faturalaştı') {
                ...
                invoiceButton.onclick = () => {
                    ...
                    // Onay butonunun tıklama olayını ayarla ve modalı göster
    ...
    // Faturalaştırma işlemini yapan ana fonksiyon (Onay modalı tarafından çağrılır)
    async function createInvoice() {
        ...
        try {
            ...
            await fetchAndRenderWaybills(); // İrsaliye listesini yenile
        } catch (error) {
...
function initializeShipmentsPage(container) {
    ...
    tableBody.addEventListener('click', async (e) => {
     ...
     // "Faturalaştır" butonunu oluştur ve dinamik olarak ekle
     const invoiceButton = document.createElement('button');
     invoiceButton.className = 'btn btn-sm btn-success';
     invoiceButton.innerHTML = `<i class="fas fa-file-invoice-dollar me-1"></i> Faturalaştır`;
    
     // Butona tıklandığında ONAY MODALINI aç
     invoiceButton.onclick = () => {
        modalTitle.textContent = 'Faturalaştırma Onayı';
     ...
     // Detay verilerini çek ve genel şablonla göster
     try {
         const response = await fetch(`${API_BASE_URL}/api/waybills/${waybillId}`, { headers: { 'Authorization': `Bearer ${token}` } });
        if (!response.ok) {
             const errorData = await response.json();
    ...
    confirmShipmentButton.addEventListener('click', async () => {
      // Artık doğru değişken olan orderToShipId'yi kontrol ediyoruz.
      if (!orderToShipId) return;
...
confirmInvoiceButton.addEventListener('click', async () => {
    ...
    try {
        ...
        invoiceModal.hide(); // Onay modalını kapat
        waybillModal.hide(); // Detay modalını da kapat
        
        // Genel başarı modalını göster
        window.showSuccessModal(`İrsaliye başarıyla faturalaştırıldı. Yeni Fatura No: ${result.invoiceNumber}`);

        // Fatura listesi sekmesini aç
...
// dashboard.js içine, diğer initialize... fonksiyonlarının yanına ekle
...
function initializePurchaseOrderListPage(container) {
    ...
    // --- 3. Ana Fonksiyon: Siparişleri Getir ve Listele ---
   async function fetchAndRenderPurchaseOrders() {
    ...
    // --- 4. Olay Dinleyicileri (Sadece bir kere kurulur) ---
    ...
    // Tablodaki "Onayla" butonuna basılmasını dinle
    tableBody.addEventListener('click', async (e) => {
       const completeBtn = e.target.closest('.complete-order-btn');
        ...
        if (completeBtn) {
            // Eğer "Teslim Al" butonuna basıldıysa (Stokları artıracak olan)
            e.stopPropagation(); 
            ...
            // Eğer diğer durum güncelleme butonlarına basıldıysa
            e.stopPropagation();
           ...
           // Eğer buton dışında bir yere (satırın kendisine) basıldıysa, detay modalını aç
           const orderId = row.dataset.id;
    ...
    // Onay modalındaki "Evet" butonuna basıldığında
    confirmButton.addEventListener('click', async () => {
        ...
        try {
            ...
            await fetchAndRenderPurchaseOrders(); // Listeyi yenile
...
function initializePurchaseWaybillsPage(container) {
   ...
   const tableBody = container.querySelector('#purchase-waybills-table-body');
    ...
    // Alış irsaliyelerini getiren ve listeleyen fonksiyon
    ...
    // İrsaliye satırına tıklandığında detayları çek ve modal'ı aç
    tableBody.addEventListener('click', async (e) => {
    ...
    async function createInvoice(waybillId, type) {
       if (!activeWaybillId) return;
        ...
        try {
            // Controller adını 'purchaseinvoices' olarak belirtiyoruz
            ...
            docModal.hide();
            window.showSuccessModal(result.message || `İrsaliye başarıyla faturalaştırıldı.`);
            await fetchAndRenderPurchaseWaybills(); // İrsaliye listesini yenile
        } catch (error) {
            modalBody.innerHTML = `<div class="alert alert-danger mt-3 p-2">${error.message}</div>`;
        } finally {
            confirmButton.disabled = false;
    ...
    // Sayfa ilk yüklendiğinde irsaliyeleri getir
...
function initializePurchaseInvoicesPage(container) {
    ...
    const docModalContent = document.getElementById('documentModalContent');
    const docModalActionButtons = document.getElementById('documentModalActionButtons');
    const printWaybillButton = document.getElementById('printDocumentButton');
    const filterInputs = container.querySelectorAll('.filter-input');
    let debounceTimer;
    ...
    // Faturaları getiren ve listeleyen fonksiyon
    async function fetchAndRenderPurchaseInvoices(search = '') {
        ...
        try {
            ...
            // Fatura satırlarına data-status attribute'u ekliyoruz
            tableBody.innerHTML = invoices.map(inv => {
                const invoiceDate = new Date(inv.invoiceDate).toLocaleDateString('tr-TR');
                const isPaid = inv.status === 'Ödendi';
                return `
    ...
    printWaybillButton.addEventListener('click', () => {
        const printableArea = document.getElementById('printable-waybill');
        if (printableArea) {
            window.print();
        } else {
    ...
    // Sayfa ilk yüklendiğinde faturaları getir
...
function initializeNewPurchaseOrderPage(container) {
    ...
    // --- Durum Değişkenleri ---
    let shoppingCart = [];
    let selectedSupplierId = null; // Cari modülü gelene kadar geçici
    let selectedProduct = null;
    let debounceTimer;
    ...
    // Ürün arama kutusuna yazıldığında API'den ürünleri çeker
     supplierSearchInput.addEventListener('keyup', (e) => {
        ...
        debounceTimer = setTimeout(async () => {
            try {
                // API'ye sadece "Tedarikçi" tipindeki carileri sor
    ...
    // ARAMA SONUÇLARINDAN BİR TEDARİKÇİ SEÇİLDİĞİNDE
    ...
    addItemBtn.addEventListener('click', () => {
         // DEĞİŞİKLİK 1: Sadece seçili ürün ve miktarı kontrol ediyoruz.
     const quantity = parseInt(productQuantityInput.value);
     ...
     // DEĞİŞİKLİK 2: Alışveriş sepetine eklenen nesneyi güncelliyoruz.
     // Fiyatı artık input'tan değil, seçilen ürünün 'purchasePrice' özelliğinden alıyoruz.
     ...
     shoppingCart.push({
        ...
        sku: selectedProduct.sku,
        quantity: quantity,
        purchasePrice: selectedProduct.purchasePrice || 0, // Ürünün alış fiyatını kullan, yoksa 0 ata.
        unitPrice : unitPrice
    });
    ...
    // Sayfa ilk açıldığında boş sepeti göster
    renderShoppingCart();
}
  ...
  function initializePriceListsPage(container) { 
    ...
    let selectedProduct = null; // Ürün aramasından seçilen ürünün ID'si
    let debounceTimer;

    // --- FONKSİYONLAR ---

    // 1. Sol taraftaki Fiyat Listelerini getiren ve çizen fonksiyon
    ...
    // 2. Seçilen bir fiyat listesinin içindeki ürünleri (kalemleri) getiren fonksiyon
    ...
    // 3. Listeye yeni bir ürün ekleme fonksiyonu
    async function addProductToList() {
        ...
        try {
            ...
            // Başarılı ekleme sonrası listeyi yenile ve formu temizle
    ...
    // Sol taraftaki bir fiyat listesine tıklandığında
    ...
    // Eğer bir liste seçilmemişse hiçbir şey yapma
    if (!selectedListId) {
    ...
    // Silme işlemi için gerekli bilgileri global değişkenlere ata
    // Bu ID, listenin kendi ID'sidir, bir ürün ID'si değil.
    itemToDeleteId = selectedListId; 
    itemToDeleteName = listName; // Mesajda göstermek için listenin adını sakla
    
    // Silme başarılı olursa, mevcut listenin içeriğini yeniden yükle
    afterDeleteCallback = () => loadPriceListItems(selectedListId, listName);
    
    // Hangi API endpoint'inin çağrılacağını ayarla. Bu, listenin TÜM ürünlerini siler.
    deleteEndpoint = `/api/pricelists/${selectedListId}/items`; 
    
    // Onay modal'ının metnini bu işleme özel olarak güncelle
    ...
    // Sağ taraftaki "Ekle" butonuna basıldığında
    ...
    searchResultsDiv.addEventListener('click', (e) => {
        ...
        const target = e.target.closest('a');
        if (target) {
            selectedProduct = JSON.parse(target.dataset.product); // Seçilen ürünün tüm bilgilerini sakla
            productSearchInput.value = selectedProduct.name; // Input'a sadece adını yaz
            searchResultsDiv.style.display = 'none'; // Arama sonuçlarını gizle
            specialPriceInput.focus(); // Fiyat alanına odaklan
        }
    });


    // Sağdaki tablodaki bir ürünü silmek için çöp kutusuna basıldığında
    itemsTableBody.addEventListener('click', (e) => {
        const deleteButton = e.target.closest('.delete-item-btn');
        if (deleteButton) {
            itemToDeleteId = deleteButton.dataset.itemId;
            itemToDeleteName = deleteButton.dataset.name;
            // Silme işlemi başarılı olursa, mevcut listenin içeriğini yenile
            afterDeleteCallback = () => loadPriceListItems(selectedListId, selectedListTitle.textContent.split("'")[1]);
            deleteEndpoint = '/api/pricelists/items'; // Özel silme endpoint'i
            deleteModalBody.textContent = `'${itemToDeleteName}' adlı ürünü bu fiyat listesinden kaldırmak istediğinize emin misiniz?`;
            deleteModal.show();
        }
    });

    // "Yeni Liste Ekle" butonuna tıklandığında (Bu, yeni bir modal açabilir veya yeni bir sekme açabilir. Şimdilik konsola yazdıralım)
    ...
    savePriceListButton.addEventListener('click', async () => {
        ...
        try {
            ...
            priceListModal.hide(); // Modal'ı kapat
            await fetchAndRenderPriceLists(); // Soldaki listeyi yenile
    ...
    // Sayfa ilk yüklendiğinde sol taraftaki fiyat listelerini getir
   ...
   function initializeNewCariPage(container) {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        // Formdaki tüm alanlardan verileri topla
        const newCariData = {
        ...
        try {
            ...
            const result = await response.json();
            if (!response.ok) {
                // Backend'den gelen validasyon hatalarını daha güzel göster
                if (response.status === 409 && result.CariKodu) { // Conflict - Kod zaten var
...
function initializeCariListPage(container) {
    ...
    let filters = {}; // Filtreleri bir obje olarak sakla
    let debounceTimer;

    // Carileri getiren ve tabloyu çizen ana fonksiyon
    async function fetchAndRenderCariler() {
        tableBody.innerHTML = `<tr><td colspan="6" class="text-center p-5"><div class="spinner-border text-primary"></div></td></tr>`;
        
        // Filtreleri URL parametresine çevir (örn: &unvan=...&cariKodu=...)
        const queryParams = new URLSearchParams(filters);
    ...
    // "Yeni Cari Ekle" butonu için olay dinleyicisi
    ...
    tableBody.addEventListener('click', (e) => {
        ...
        if (deleteButton){
            itemToDeleteId = deleteButton.dataset.id;
            itemToDeleteName = deleteButton.dataset.name;
            afterDeleteCallback = fetchAndRenderCariler; // Silme sonrası listeyi yenile
            deleteEndpoint = '/api/cariler'; // Silme için doğru endpoint'i ayarla
    ...
    // Sayfa ilk yüklendiğinde carileri getir
    fetchAndRenderCariler();
}
function initializeCariEditPage(container, pageUrl) {
    ...
    // API'den carinin mevcut verilerini çeker ve formu doldurur
    ...
    // Form gönderildiğinde güncelleme isteği at
    form.addEventListener('submit', async (e) => {
        e.preventDefault();
        saveButton.disabled = true;
        saveButton.innerHTML = `<span class="spinner-border spinner-border-sm"></span> Güncelleniyor...`;
        ...
        try {
            ...
            // Güncellemeden sonra cari listesi sekmesi açıksa, onu yenileyelim.
...
function initializeCariTransactionsPage(container) {
    // --- 1. EKSİK OLAN TEMEL AYARLAR EKLENDİ ---
    const token = localStorage.getItem('authToken');
    if (!token) { window.location.href = 'invlogin.html'; return; }
    // API_BASE_URL'in de global olarak tanımlı olduğunu varsayıyoruz.
    
    // --- 2. HTML Elemanlarını Seçme (Kodunuzdakiyle Aynı) ---
    ...
    // Arama kutusuna yazıldığında Carileri getir
    searchInput.addEventListener('keyup', (e) => {
        ...
        debounceTimer = setTimeout(async () => {
            try {
                // Not: Bu API endpoint'i hem müşteri hem tedarikçi getirmelidir
    ...
    // Arama sonuçlarından bir Cari seçildiğinde ekstreyi getir
    searchResults.addEventListener('click', async (e) => {
        ...
        // Arayüzü güncelle ve yükleniyor durumuna getir
        searchInput.value = cariName;
        searchResults.style.display = 'none';
        selectedCariNameEl.textContent = cariName;
        tableBody.innerHTML = `<tr><td colspan="5" class="text-center p-5"><div class="spinner-border"></div></td></tr>`;
        summaryPanel.style.display = 'none'; // Önceki özeti gizle

        try {
            // --- 4. DOĞRU API İSTEĞİ ---
            // Backend'den bu cariye ait birleşik ekstre verisini çekmek için ReportsController'ı kullanıyoruz.
            ...
            let balance = 0;
            let totalDebit = 0; // Toplam Borç
            let totalCredit = 0; // Toplam Alacak
...
function initializeNewCollectionSlipPage(container) {
    ...
    // Hata Kontrolü
    if (!form || !accountSelect || !dateInput || !invoiceSearchInput) {
        console.error("Yeni Tahsilat Fişi sayfasındaki temel HTML elemanları bulunamadı. Lütfen HTML dosyasını ve ID'leri kontrol edin.");
        return;
    }

    // --- 2. Yardımcı Fonksiyonlar ---
    // Sayfa açıldığında Kasa/Banka hesaplarını çek ve dropdown'ı doldur
    ...
    // Fatura arama kutusuna yazıldığında
    invoiceSearchInput.addEventListener('keyup', (e) => {
        ...
        debounceTimer = setTimeout(async () => {
            try {
                // Backend'den durumu "Ödenecek" olan Satış Faturalarını çek (Müşteri veya Fatura No'ya göre)
                // Not: InvoicesController kullandığımız için 'invoices' adresine gidiyoruz.
                const params = new URLSearchParams({ status: 'Ödenecek', customerName: searchTerm, invoiceNumber: searchTerm });
                const response = await fetch(`${API_BASE_URL}/api/invoices?${params.toString()}`, { headers: { 'Authorization': `Bearer ${token}` } });
                if (!response.ok) throw new Error('Faturalar aranırken bir hata oluştu.');
                const invoices = await response.json();
                
                if (invoices && invoices.length > 0) {
                    // DİKKAT: Backend artık 'customerName' gönderdiği için burası doğru çalışacak
                ...
                invoiceSearchResults.innerHTML = `<span class="list-group-item text-danger">${error.message}</span>`;
                invoiceSearchResults.style.display = 'block';
            }
        }, 500);
    });

    // Arama sonuçlarından bir fatura seçildiğinde
    ...
    form.addEventListener('submit', async (e) => {
        ...
        if (!accountSelect.value) {
            alert("Lütfen tahsilatın yapılacağı hesabı seçin.");
            return;
        }
        if (!dateInput.value) {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/api/financialtransactions`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
    ...
    // --- 4. İlk Yükleme ---
    dateInput.value = new Date().toISOString().split('T')[0]; // Tarihi bugüne ayarla
    populateAccounts(); // Kasa/Banka hesaplarını doldur
}

function initializeNewPaymentSlipPage(container) {
    const token = localStorage.getItem('authToken');
    if (!token) { window.location.href = 'invlogin.html'; return; }
    ...
    // Hata Kontrolü
    if (!form || !accountSelect || !dateInput || !invoiceSearchInput) {
        console.error("Yeni Ödeme Fişi sayfasındaki temel HTML elemanları bulunamadı. Lütfen HTML dosyasını ve ID'leri kontrol edin.");
        return;
    }
    ...
    invoiceSearchInput.addEventListener('keyup', (e) => {
        ...
        debounceTimer = setTimeout(async () => {
            try {
                // --- DEĞİŞİKLİK BURADA: DOĞRU API VE DOĞRU PARAMETRELER KULLANILIYOR ---
                const params = new URLSearchParams({ 
                    supplierName: searchTerm, 
                    invoiceNumber: searchTerm 
                });
                // API adresi: /api/purchaseinvoices
                const response = await fetch(`${API_BASE_URL}/api/purchaseinvoices?${params.toString()}`, { headers: { 'Authorization': `Bearer ${token}` } });
                if (!response.ok) throw new Error('Faturalar aranırken bir hata oluştu.');
                const invoices = await response.json();
                
                if (invoices && invoices.length > 0) {
                    // DİKKAT: Backend'den 'supplierName' geldiği için bunu kullanıyoruz
                    invoiceSearchResults.innerHTML = invoices.map(inv => 
                        `<a href="#" class="list-group-item list-group-item-action" 
                            data-invoice-id="${inv.id}" 
                            data-invoice-amount="${inv.totalAmount}" 
                     ...
                     if (!invoiceSearchResults.innerHTML.trim()) {
                         invoiceSearchResults.innerHTML = '<span class="list-group-item disabled">Aramanıza uygun, durumu "Ödenecek" olan fatura bulunamadı.</span>';
                        }
                } else {
                    invoiceSearchResults.innerHTML = '<span class="list-group-item disabled">Ödenecek fatura bulunamadı</span>';
                }
    ...
    form.addEventListener('submit', async (e) => {
        ...
        if (!accountSelect.value) {
            alert("Lütfen ödemenin yapılacağı hesabı seçin.");
            return;
        }
        if (!dateInput.value) {
            alert("Lütfen işlem tarihini seçin.");
        ...
        const transactionData = {
            transactionDate: dateInput.value,
            transactionType: "Ödeme", // DİKKAT: İşlem tipi "Ödeme"
            cashAndBankAccountId: parseInt(accountSelect.value),
            relatedInvoiceId: selectedInvoiceId,
            description: descriptionInput.value
        };
...
// YENİ FONKSİYON: Stok Hareketleri Sayfası
function initializeStockMovementsPage(container) {
    ...
    // Hareketleri getiren ve listeleyen ana fonksiyon (artık filtreleri kabul ediyor)
    async function fetchAndRenderMovements(filters = {}) {
        tableBody.innerHTML = `<tr><td colspan="5" class="text-center p-5"><div class="spinner-border text-primary"></div></td></tr>`;
        
        // Filtre nesnesinden bir URL sorgu dizesi oluştur
        const params = new URLSearchParams();
    ...
    // Filtrele butonuna basıldığında formu dinle
    ...
    // Sayfa ilk yüklendiğinde tüm hareketleri getir
...
function initializeGoodsReceiptPage(container) {
    ...
    async function fetchAndRenderIncomingOrders() {
        ...
        // API'den SADECE durumu "Gönderildi" olan siparişleri çekiyoruz
    ...
    // --- 3. Olay Dinleyicileri ---
      ...
      tableBody.addEventListener('click', (e) => {
        ...
        if (receiveBtn) {
            ...
            // Eğer "İptal Et" butonuna basıldıysa (Stoğu etkilemeyecek olan)
    ...
    // Onay modalındaki "Evet" butonu artık her iki işlem için de çalışır
    confirmButton.addEventListener('click', async () => {
        if (!orderToUpdateId || !newStatusToSet) return;
...
function initializeMonthlyTurnoverReportPage(container) {
    ...
    async function fetchAndRenderReport() {
        try {
            // Backend'deki GetMonthlyTurnoverReport metodunu bu URL ile çağırıyoruz
            const response = await fetch(`${API_BASE_URL}/api/reports/monthly-turnover`, { headers: { 'Authorization': `Bearer ${token}` } });
            
            // Eğer cevap başarılı değilse (örn: 500 hatası), bir hata fırlat
            if (!response.ok) {
            ...
            // Eğer daha önce bir grafik çizildiyse, onu temizle
            if (chartCanvas.chart) {
                chartCanvas.chart.destroy();
            }
            
            // Chart.js ile yeni bir birleşik grafik oluştur
            chartCanvas.chart = new Chart(chartCanvas, {
                type: 'bar', // Ana grafik tipi
                data: {
                    ...
                    datasets: [
                        {
                            ...
                            type: 'line', // Bu veri setini çizgi grafik olarak göster
                            borderColor: '#1cc88a', // Yeşil
                            backgroundColor: 'rgba(28, 200, 138, 0.1)',
                            fill: true,
                            tension: 0.3,
                            order: 0 // Çizginin en üstte olmasını sağla
                        },
                        {
                            label: 'Toplam Satış (₺)',
    ...
    // Excel'e aktar butonuna basıldığında
            ...
            if (!transactions || transactions.length === 0) {
                tableBody.innerHTML = `<tr><td colspan="5" class="text-center p-4">Kriterlere uygun kayıt bulunamadı.</td></tr>`;
                 ...
                 tableBody.innerHTML = transactions.map(t => {
                    ...
                    return `
                        <tr>
                            <td>${new Date(t.transactionDate).toLocaleString('tr-TR', { dateStyle: 'short', timeStyle: 'short' })}</td>
...
function initializeProductSalesReportPage(container) {
    ...
    // Detay butonlarına tıklanmasını dinle
    tableBody.addEventListener('click', async (e) => {
...
// YENİ FONKSİYON: Müşteri Satış Raporu Sayfası
function initializeCustomerSalesReportPage(container) {
        ...
        try {
            ...
            tableBody.innerHTML = reportData.map(item => `
                <tr>
                    ...
                    <td class="text-center">${new Date(item.lastPurchaseDate).toLocaleString('tr-TR', { dateStyle: 'short', timeStyle: 'short' })}</td>
    ...
    fetchAndRenderReport(); // Sayfa ilk yüklendiğinde tüm veriyi getir
}
...
// dashboard.js içindeki bu fonksiyonun tamamının bu şekilde olduğundan emin ol
...
function initializeCariActivitiesPage(container) {
    ...
    // Hata kontrolü: Eğer bir eleman bulunamazsa, hatayı konsola yaz ve fonksiyonu durdur.
    if (!filterForm) {
        console.error("KRİTİK HATA: '#cari-activities-filter-form' ID'li form bulunamadı. Yüklenen HTML doğru mu?");
        return; // Bu return komutu 'addEventListener' hatasını önler.
    }
    if (!tableBody) {
    ...
    // --- 2. Ana Fonksiyon: Hareketleri Getir ve Listele ---
    ...
    async function fetchAndRender(filters = {}) {
        ...
        try {
            ...
            tableBody.innerHTML = activities.map(item => {
                ...
                return `
                    <tr>
                        <td>${new Date(item.documentDate).toLocaleString('tr-TR', { dateStyle: 'short', timeStyle: 'short' })}</td>
                        <td>
...
function initializeCashBankAccountsPage(container) {
    ...
    // --- 2. Ana Fonksiyon: Hesapları Getir ve Listele ---
    ...
    // --- 3. Olay Dinleyicisi: Yeni Hesap Ekleme Formu ---
    form.addEventListener('submit', async (e) => {
        e.preventDefault();
        ...
        const newAccountData = {
            ...
            initialBalance: 0 // Başlangıç bakiyesi varsayılan olarak 0
        };
        ...
        try {
            ...
            if (!response.ok) {
                ...
                // Hata mesajını daha detaylı göstermek için (opsiyonel)
                const errorMessages = Object.values(errorResult.errors || {}).flat().join('\n');
                throw new Error(errorMessages || 'Hesap oluşturulamadı.');
            }
            
            form.reset(); // Formu temizle
            currencyInput.value = 'TRY'; // Para birimini varsayılana döndür
            await fetchAndRenderAccounts(); // Tabloyu yenile
...
// Bu yeni fonksiyonu dashboard.js'e, diğer initialize... fonksiyonlarının altına ekle
...
function initializeAccountStatementPage(container) {
    ...
    let accountsData = []; // Hesapların tam verisini (başlangıç bakiyesi dahil) saklamak için

    // Para formatlama yardımcısı
    const currencyFormatter = new Intl.NumberFormat('tr-TR', { style: 'currency', currency: 'TRY' });

    // --- 2. Ana Fonksiyon: Hesap Ekstresini Getir ve Listele ---
    async function fetchAndRenderStatement(accountId) {
        ...
        // Seçilen hesabın başlangıç bakiyesini bul
            ...
            tableBody.innerHTML = transactions.map(t => {
                ...
                let outflow = 0;
                 if (t.transactionType === 'Tahsilat' || t.transactionType === 'Virman Giriş') {
                    inflow = t.amount;
                } else { // 'Ödeme' veya 'Virman Çıkış'
                    outflow = t.amount;
                }

                runningBalance += inflow - outflow; // Yürüyen bakiye hesabı
    ...
    // --- 3. Yardımcı Fonksiyon: Hesapları Çek ve Dropdown'ı Doldur ---
    async function populateAccounts() {
        try {
            const response = await fetch(`${API_BASE_URL}/api/cashandbankaccounts`, { headers: { 'Authorization': `Bearer ${token}` } });
            accountsData = await response.json(); // Gelen veriyi global değişkene kaydet
    ...
    // --- 5. İlk Yükleme ---
    populateAccounts(); // Sayfa açıldığında önce hesapları doldur
}
// dashboard.js içindeki bu fonksiyonu bul ve tamamını değiştir
...
function initializeAccountTransferPage(container) {
    ...
    // --- 2. Yardımcı Fonksiyon: Hesapları Çek ve Dropdown'ları Doldur ---
    async function populateAccounts() {
        try {
            const response = await fetch(`${API_BASE_URL}/api/cashandbankaccounts`, { headers: { 'Authorization': `Bearer ${token}` } });
            if (!response.ok) throw new Error('Hesaplar yüklenemedi.');
    ...
    // --- 3. Form Gönderme Olay Dinleyicisi ---
    form.addEventListener('submit', async (e) => {
        e.preventDefault();
    ...
    // --- 4. İlk Yükleme ---
    dateInput.value = new Date().toISOString().split('T')[0]; // Tarih alanını bugünün tarihiyle doldur
    populateAccounts(); // Sayfa açıldığında hesapları doldur
}
...
function initializeAuditLogPage(container) {
    ...
    // Değişiklikler kolonunu daha okunaklı hale getiren yardımcı fonksiyon
    function formatChanges(changesJson) {
        try {
            const changes = JSON.parse(changesJson);
            // Sadece CurrentValue'su olan property'leri gösterelim
            return changes.filter(c => c.CurrentValue !== null)
                .map(c => `<div><strong>${c.Name}:</strong> ${JSON.stringify(c.CurrentValue)}</div>`)
                .join('');
        } catch (e) {
            return changesJson; // Eğer JSON değilse, ham metni göster
        }
    }

    // --- 2. Ana Fonksiyon: Logları Getir ve Listele ---
...
function initializeSettingsPage(container) {
    ...
    // Başlangıçta profil bilgilerini çek ve formu doldur
    ...
    async function loadCompanyProfile() {
        try {
            ...
            // Eğer kullanıcı daha önce profil oluşturmuşsa, form alanlarını doldur
            if (profile) {
    ...
    companyProfileForm.addEventListener('submit', async (e) => {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/api/companyprofile`, {
                method: 'POST', // Hem oluşturma hem güncelleme için POST kullanıyoruz
                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
                body: JSON.stringify(data)
            });
            const result = await response.json();
...
function initializeSupportPage(container) {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        // Form verilerini toplamak için FormData kullanıyoruz, çünkü dosya eki var.
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/api/support/send-ticket`, {
                method: 'POST',
                headers: {
                    // DİKKAT: FormData kullanırken 'Content-Type' belirtmiyoruz.
                    // Tarayıcı bunu bizim için otomatik olarak ayarlar.
                    'Authorization': `Bearer ${token}`
                },
                body: formData
            });
...
function initializeProfilePage(container) {
    ...
    async function loadProfile() {
        try {
            ...
            // Verileri HTML elemanlarına yazdır
            ...
            detailRole.textContent = profile.role; // YENİ
    ...
    // "Ayarları Düzenle" butonuna basıldığında Ayarlar sekmesini aç
    goToSettingsBtn.addEventListener('click', () => {
        // Global openTab fonksiyonunu kullanarak Ayarlar sekmesini açıyoruz
...
applyRoleBasedUI();
    ...
    window.openTab = async function (pageUrl, pageTitle) {
        ...
        try {
            ...
            if (pageUrl === 'dashboard-summary') {
                ...
                initializeNewProductForm(tabPane); // YENİ EKLENDİ
            }
            else if (pageUrl.includes("kategoriler.html")) { // BU SATIRI EKLE
                   initializeCategoriesPage(tabPane);            // BU SATIRI EKLE
            }
            else if (pageUrl.includes("markalar.html")) { // YENİ EKLENEN BLOK
                    initializeBrandsPage(tabPane);
            }
            else if (pageUrl.includes("birimler.html")) { // YENİ EKLENEN BLOK
                   initializeUnitsPage(tabPane);
            }
            else if (pageUrl.includes("fiyat-listeleri.html")) { // YENİ EKLENEN BLOK
                   initializePriceListsPage(tabPane);
            }
            else if (pageUrl.includes("urun-islemleri.html")) { // YENİ EKLENEN BLOK
                   initializeProductEditPage(tabPane, pageUrl); 
             }
            else if (pageUrl.includes("satis-siparisleri.html")) {
                   initializeOrderListPage(tabPane);
            }
            else if (pageUrl.includes("yeni-satis-siparisi.html")) { // BU BLOK EKLENECEK
                   initializeNewOrderPage(tabPane);
             }
            else if (pageUrl.includes("sevk-edilecekler.html")) { // BU BLOK EKLENECEK
                 initializeShipmentsPage(tabPane);
             }
            else if (pageUrl.includes("satis-irsaliyeleri.html")) { // BU BLOK EKLENECEK
            ...
            else if (pageUrl.includes("satis-faturalari.html")) { // YENİ EKLENEN BLOK
                  initializeInvoicesPage(tabPane);
             }
            else if (pageUrl.includes("alis-irsaliyeleri.html")) { // YENİ EKLENEN BLOK
                  initializePurchaseWaybillsPage(tabPane);
            } 
            else if (pageUrl.includes("alis-faturalari.html")) { // YENİ EKLENEN BLOK
               initializePurchaseInvoicesPage(tabPane);
            }
            else if (pageUrl.includes("cari-listesi.html")) { // YENİ EKLENEN BLOK
              initializeCariListPage(tabPane);
            }  
            else if (pageUrl.includes("yeni-cari.html")) { // YENİ EKLENEN BLOK
              initializeNewCariPage(tabPane);
            }
            else if (pageUrl.includes("cari-duzenle.html")) { // YENİ EKLENEN BLOK
             initializeCariEditPage(tabPane, pageUrl);
            }
            else if (pageUrl.includes("cari-hareketleri.html")) { // YENİ EKLENEN BLOK
              initializeCariTransactionsPage(tabPane);
            }
            else if (pageUrl.includes("yeni-tahsilat.html")) { // YENİ EKLENEN BLOK
             initializeNewCollectionSlipPage(tabPane);
            }
            else if (pageUrl.includes("yeni-odeme.html")) { // YENİ EKLENEN BLOK
              initializeNewPaymentSlipPage(tabPane);
            } 
            else if (pageUrl.includes("borc-alacak-raporu.html")) { // YENİ EKLENEN BLOK
              initializeBorcAlacakReportPage(tabPane);
            }
            else if (pageUrl.includes("stok-durum-raporu.html")) { // YENİ EKLENEN BLOK
             initializeStockStatusReportPage(tabPane);
            }
            else if (pageUrl.includes("stok-hareketleri.html")) { // YENİ EKLENEN BLOK
              initializeStockMovementsPage(tabPane);
            }
            else if (pageUrl.includes("mal-kabul.html")) { // YENİ EKLENEN BLOK
              initializeGoodsReceiptPage(tabPane);
            }
            else if (pageUrl.includes("aylik-ciro-raporu.html")) { // YENİ EKLENEN BLOK
              initializeMonthlyTurnoverReportPage(tabPane);
            }
            else if (pageUrl.includes("urun-satis-raporu.html")) { // YENİ EKLENEN BLOK
               initializeProductSalesReportPage(tabPane);
            }
            else if (pageUrl.includes("tahsilat-odeme-raporu.html")) { // YENİ EKLENEN BLOK
               initializeFinancialReportPage(tabPane);
            }
            else if (pageUrl.includes("musteri-satis-raporu.html")) { // YENİ EKLENEN BLOK
               initializeCustomerSalesReportPage(tabPane);
            }
            else if (pageUrl.includes("karlilik-raporu.html")) { // YENİ EKLENEN BLOK
            ...
            else if (pageUrl.includes("siparis-hareketleri.html")) { // YENİ EKLENEN BLOK
                initializeOrderActivitiesPage(tabPane);
            }
            else if (pageUrl.includes("kasa-banka-tanimlari.html")) { // YENİ EKLENEN BLOK
               initializeCashBankAccountsPage(tabPane);
             }
             else if (pageUrl.includes("kasa-hareketleri.html")) { // YENİ EKLENEN BLOK
               initializeAccountStatementPage(tabPane);
            }
            else if (pageUrl.includes("hesaplar-arasi-virman.html")) { // YENİ EKLENEN BLOK
               initializeAccountTransferPage(tabPane);
            }
            else if (pageUrl.includes("islem-kayitlari.html")) { // YENİ EKLENEN BLOK
              initializeAuditLogPage(tabPane);
            }
            else if (pageUrl.includes("ayarlar.html")) { // YENİ EKLENEN BLOK
               initializeSettingsPage(tabPane);
            }
            else if (pageUrl.includes("destek.html")) { // YENİ EKLENEN BLOK
              initializeSupportPage(tabPane);
            }
            else if (pageUrl.includes("profilim.html")) { // YENİ EKLENEN BLOK
    ...
    document.querySelectorAll(".sub-item").forEach(item => {
        item.addEventListener("click", function(e) {
            ...
            // Genel Özet için özel bir durum kontrolü yok, direkt openTab çağrılır.
            // openTab fonksiyonu zaten bu durumu yönetecek.
            if (pageUrl) {
                window.openTab(pageUrl, pageTitle);
            }
        });
    });
   const homePageContainer = document.getElementById('tab-ozet');
    if (homePageContainer) {
        initializeHomePage(homePageContainer);
    }
